...
static void freeHashList(h_ptr head)
{
    while (head != 0)
    {
...
// Comparator function
static int comparator(const void *vp1, const void *vp2)
{
    const h_ptr h1 = *(h_ptr *)vp1;
    const h_ptr h2 = *(h_ptr *)vp2;
...
h_ptr sort_words()
{
    ...
    // Quick Sort
    qsort(array, cnt, sizeof(ele), comparator);
